...
export function App() {
  ...
  const [mode, setMode] = useState<{
    ...
    symbol: { type: 'interval', rootNote: 'C', muted:[], fingerAssignments: [], enharmonicMode: 'sharp'},
        ...
        case 'frets': {
          setMode((mode) => {
            if (mode.highlight.type !== 'frets') {
              return update(mode, {
                highlight: {
                  $set: {type:'frets', assignments: [{string: stringIndex, fret}]} 
                }
              })
            }
            return update(mode, {
              highlight: {
                assignments: {$apply: (assignments: {string: number, fret: number}[]) => {
                  if (assignments.find((assignment) => assignment.string === stringIndex && assignment.fret === fret)) {
                    return assignments.filter((assignment) => assignment.string !== stringIndex || assignment.fret !== fret);
                  }
            ...
            const symbol = update(mode.symbol, {
              fingerAssignments: {
                $apply: (fingerAssignments: FingerAssignment[]) => {
                  const hasFinger =fingerAssignments.find((assignment) => assignment.finger === interactionMode.finger)
                  return (hasFinger ? fingerAssignments : [...fingerAssignments, {finger: interactionMode.finger, assignments:[{string: stringIndex, fret}]}]).map((fingerAssignment) => {
  ...
  return (
    <div>
      <Select label="Instrument: " value={instrument} values={InstrumentList} onSelect={(instrument) => {setInstrument(instrument as Instrument)}} />
      <Select label="Symbol Mode: " value={mode.symbol.type} values={["interval", "note"]} onSelect={(type) => {
        setMode(mode => update(mode, {symbol:{type:{$set: type as 'interval'|'note'}}}))}} />
      {mode.symbol.type === 'interval' && (
        <Select label="Root Note: " value={mode.symbol.rootNote} values={RootNoteList.slice()} onSelect={(rootNote) => {
          setMode(mode => update(mode, {symbol:{rootNote:{$set: rootNote}}}))}} />
      )}
      <Neck
