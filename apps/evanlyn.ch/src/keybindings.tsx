...
const setContent = (child:Trunk, content:string, dispatch:Dispatch<Action>) => {
    if (child.value === undefined){
        return
    }
    dispatch({type:"MODIFY",
...
const KeyDownHandler = (child:Trunk, head:Trunk, mode:string, setMode:(mode:string)=>void, dispatch:Dispatch<Action>) => (e:KeyboardEvent) => {
    switch (mode){
        case 'vim-default':
...
const keyDownCommon = (e:KeyboardEvent, child:Trunk, setMode:(mode:string)=>void, dispatch:Dispatch<Action>) => {
...
const keyDownNote = (e:KeyboardEvent, child:Trunk, setMode:(mode:string)=>void, dispatch:Dispatch<Action>) => {
...
const keyDownVimDefault = (e:KeyboardEvent, child:Trunk, head:Trunk, setMode:(mode:string)=>void, dispatch:Dispatch<Action>) => {
    if (e.metaKey){
        return;
    }
    e.preventDefault();
...
const keyDownVimInput = (e:KeyboardEvent, child:Trunk, setMode:(mode:string)=>void, dispatch:Dispatch<Action>) => {
    if (e.keyCode === 27){ //'Escape'){
        e.preventDefault();
        setMode('vim-default');
    }
...
const keyDownStandard = (e:KeyboardEvent, child:Trunk, setMode:(mode:string)=>void, dispatch:Dispatch<Action>) => {
