...
const Main = (props:{initialState:MainState}) => {
    const [mainState, dispatch] = useReducer<Reducer<MainState, Action>>(rootReducer, props.initialState);
...
const renderMagnolia = () => {
    ...
    const render = (trunk:Trunk|PartialTrunk, whose:"mine"|"yours"|"secret") => {
        root.render(
            <Main initialState={{
                magnolia: {
                    tree: ParseTrunk(trunk, () => ({title:"", link:undefined, content:undefined})),
                    headSerial: initHead,
    ...
    if (whose === "mine") {
        getJSON(mglFile, (trunk:Trunk) => {
            render(trunk, "mine");
        }, ()=>{
            //console.log("failed to load trunk");
        });
    } else if (whose === "yours") {
        const trunk = JSON.parse(window.localStorage.getItem('trunk') || 'false') || MakeEmptyTree( () => ({title:"", link:undefined, content:undefined}));
        render(trunk, "yours");
    } else if (whose === "secret") {
        getJSON("https://boardzorg.org/zorg/trunk", (data)=>{
