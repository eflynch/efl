...
function Magnolia():JSX.Element {
    ...
    useEffect(()=>{
        ...
        if (!head.value?.content && (head.value?.note === undefined) && head.childs.length === 0 && parent!== undefined) {
    ...
    const setCollapsed = (child:Trunk, collapsedState:boolean) => {
        dispatch({type:"SET_COLLAPSED", child:child, collapsed:collapsedState});
    };
    ...
    const setNotes = (child:Trunk, note:string) => {
        ...
        const value = child.value
        dispatch({type:"MODIFY", child:child, value:update(value, {note: {$set: note}})});
    };
            ...
            <div style={{
                ...
                <div>
                    ...
                    : head?.childs.map((child:Trunk|undefined) =>{
                        ...
                        const focusAncestors = AncestorsOf(tree, focus).slice(1 + numHeadAncestors);
